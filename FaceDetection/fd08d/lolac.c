...
float SR = 36.0/32.0; // ((float)SW)/((float)SH);
	...
	//GLOBAL VARIABLES
struct CNN gl_cnn;
//#pragma omp threadprivate (gl_cnn)
...
void torc_phase1(int iter, int itotal,
	...
	void *data_ptr, int *Sm, int *facesFound
)
{
	int i, S, SH = 36;
	float SR = 36.0 / 32.0; // ((float) SW / ((float)SH);
	...
	// Single iteration (64 in total in our case)
...
void torc_phase3(void *data_ptr, int n, int n_end,
	void *img_input_ptr, void *cnn_ptr, int *allSize)
{
...
int *W, float *O)
{
	...
	for (iter = 0; iter < itotal; iter++)
	{
		...
		torc_phase1( iter, itotal,
				(void *) img_source, (void *) img_input,
				(void *) data, &Sm, &facesFound);
	}

	#pragma ompix tasksync
	//printf("tasksync completed\n"); fflush(0);
	// No need to do this in shared memory environments.
	// However, we must do this in cases of distributed memory
	// systems.
	// TODO - Make some sense out of this shit
	/*
	for (i = 0; i < 64; i++){
		if (task_results[i].S){
			// Copy its results to data->...
			data->     Xs[result_cnt] = task_results[i].xcenter;
			data->     Ys[result_cnt] = task_results[i].ycenter;
			data->heights[result_cnt] = task_results[i].S;
			data->outputs[result_cnt] = task_results[i].output;
			result_cnt++;
			printf("Results #%d:\n%d %d %d %.3f\n", result_cnt,
				task_results[i].xcenter,
				task_results[i].ycenter,
				task_results[i].S,
				task_results[i].output);
		}
	}
	*/
	...
	/* Second phase: VoteGrouping2. Too short */;
	t2 = my_gettime();
...
///////////////////////////////////////////////////////////////////////////////
	...
	//#pragma ompix task
	//torc_phase3(data, n, allClusters, img_input, cnn, &allSize);
	...
	{
        #pragma ompix task atnode(here)
        torc_phase3(data, n, n + 1, img_input, cnn, &allSize);
        fflush(0);
        continue;
		//the various scales to try (Hf is the current target's height)
		float S11 = 0.8*data->Hf[n];
...
//////////////////////////////////////////////////////////////////////////////////////////// ALLCLUSTERS FOR END
	...
	/* -Parallel output? -Not so critical */
	t5 = my_gettime();
    ...
    /*
	printf("phase 1: %f seconds\n", t2 - t1);
	printf("phase 2: %f seconds\n", t3 - t2);
	printf("phase 3: %f seconds\n", t4 - t3);
	printf("phase 4: %f seconds\n", t5 - t4);
	printf("phase 5: %f seconds\n", t6 - t5);
