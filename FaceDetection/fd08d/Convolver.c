...
//			k=3;
	...
	//just follow the path od ConvolveRoughly but just do the memory allocations
...
void torc_conv_task0(int i, int width, int height,
					void *CConvolver_v_ptr, void *img_ptr, // CConvolver *, lpiImage *, respectively
...
#pragma ompix taskdef in(i, width, height, current_width, current_height) inout(CConvolver_v_ptr[1], img_ptr[1])
void torc_conv_task1(int i, int width, int height,
						 void *CConvolver_v_ptr, void *img_ptr,
					  int current_width, int current_height)
{
...
#pragma ompix taskdef in(i, width, height, current_width, current_height) inout(CConvolver_v_ptr[1], img_ptr[1])
void torc_conv_task2(int i, int width, int height,
	void *CConvolver_v_ptr, void *img_ptr,
	int current_width, int current_height)
{
	struct lpiImage *fm1_tmp;
...
#pragma omp parallel for schedule(static,1)
	...
	{
#ifdef L2PARTASK_LO1
		#pragma ompix task untied
		torc_conv_task0(i, width, height, (void *) CConvolver_v, (void *) img, current_width, current_height);
#else
		//{
		struct lpiImage *fm0_tmp;
...
#ifdef L2PARTASK_LO2
		#pragma ompix task untied
		torc_conv_task1(i, width, height, (void *) CConvolver_v, (void *) img, current_width, current_height);
#else
		{
		struct lpiImage *fm1_tmp;
		fm1_tmp=CConvolver_CreateImage(current_width-2, current_height-2);
...
#pragma omp parallel for schedule(static,1)
	...
	for(i=8;i<14;i++)
	{
		...
		torc_conv_task2(i, width, height, (void *) CConvolver_v, (void *) img, current_width, current_height);
#else
		{
		struct lpiImage *fm1_tmp;
		fm1_tmp=CConvolver_CreateImage(current_width-2, current_height-2);
