...
//	r_width = ROUND_UP(width, 4);
//	r_height = ROUND_UP(height, 4);
//#if defined(PARBATCH)
//	r_width = ROUND_UP(width, 32);
//	r_height = ROUND_UP(height, 36);
//#endif
	res->imageData=(unsigned char *)calloc((r_width)*(r_height)+64, block_size);
//	res->imageData=(unsigned char *)memalign(64, (r_width)*(r_height)*block_size);
	return(res);
}
